...
export default function Divide() {
    ...
    const generate_equation = () =>{
       setQuiz((prevQuiz)=>{
        ...
        let newNum2 = Math.floor(Math.random() * 10);
        
        // always big number is front then subtract 
        if(newNum1 < newNum2){
            [newNum1,newNum2] = [newNum2,newNum1];
        } 
        // if newnum2 is 0 then add 1 
        if(newNum2 == 0){
            newNum2 = 1;
        }
    ...
    const submitAns = (e) =>{
        // get data-val from button attribute then compare and equation 
        const userAns = e.target.getAttribute('data-val');
        if(userAns == Quiz.ans){
            Quiz.score++;
            generate_equation();
        }else{
                ...
                <div className=" space-x-8">
                    {
                        Quiz.allAnswers.map((answer,i) => (
                            <button onClick={submitAns} key={i} data-val={answer} className='bg-[rgb(167,182,29)] text-white text-5xl font-Luckiest rounded-xl px-7 py-5 md:px-10 md:py-6 '>
                            {
                                // if number int is false then toFixed 2
